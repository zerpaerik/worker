...
class MakeOut extends StatefulWidget {
  final User? user;
  final int? workday;
  ...
  @override
  _MakeOutState createState() =>
      _MakeOutState(user!, workday!, contract!, work!, wk!);
}

class _MakeOutState extends State<MakeOut> {
  Map<String, dynamic> contract;
  User user;
  int workday;
  Workday work;
  ...
  _MakeOutState(this.user, this.workday, this.contract, this.work, this.wk);
  ...
  Future<void> _editWorkdayDef() async {
    ...
    try {
      Provider.of<WorkDay>(context, listen: false)
          .makeClockOut(_user!.id, start_time, hourClock, widget.workday,
              workday_on!, geo)
          .then((response) {
        setState(() {
          isLoading = false;
