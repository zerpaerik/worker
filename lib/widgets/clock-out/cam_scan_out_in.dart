...
import '../clock-in/cam_scan.dart';
import '../clock-in/detail.dart';
import '../clock-in/list.dart';
import '../global.dart';
import 'detail.dart';
import 'detail_scan_out_in.dart';
import 'list.dart';

class QRSCANINOUT extends StatefulWidget {
  final User? user;
  final int? workday;
...
class _QRSCANState extends State<QRSCANINOUT> {
  User user;
  int workday;
  Map<String, dynamic> contract;
  Workday work;
  ...
  Future<bool?> scanQRWorker(
    ...
    if (response.statusCode == 200 && resBody['first_name'] != null) {
      ...
      Navigator.push(
        context,
        MaterialPageRoute(
            builder: (context) => DetailClockInOut(
                  user: _user,
                  workday: widget.workday,
                  lat: lat,
                  long: long,
