...
import 'package:worker/widgets/workday-report/editm/lunch.dart';

import '../../../model/user.dart';
import 'package:intl/intl.dart' show DateFormat;

class EditWorkdayM1 extends StatefulWidget {
  ...
  @override
  _EditWorkdayM1State createState() => _EditWorkdayM1State(
      user, workday, contract, wr, report, worker, workerDescription);
}

class _EditWorkdayM1State extends State<EditWorkdayM1> {
  User? user;
  int? workday;
  Map<String, dynamic>? contract;
  WorkdayRegister? wr;
  ...
  List? worker = [];
  List? workerDescription = [];
  _EditWorkdayM1State(this.user, this.workday, this.contract, this.wr,
  ...
  Future<dynamic> _submit() async {
    ...
    try {
      Provider.of<WorkDay>(context, listen: false)
          .editWorkdayReportM1(widget.workday, hourClock, hourClock1,
              widget.worker, start_time, end_time)
          .then((response) {
        setState(() {
          isLoading = false;
        });
                        ...
                        child: Column(
                          children: <Widget>[
                            Text(
                              end_time != null
                                  ? DateFormat("MMMM d yyyy").format(
                                      DateTime.parse(end_time.toString()))
                                  : 'S/F',
                              style: TextStyle(fontSize: 16),
                            ),
                            Text(
                ...
                child: isLoading
                    ...
                    : ElevatedButton(
                        ...
                        onPressed:
                            ...
                            : null*/
                            () {
                          if (start_time!.isAtSameMomentAs(end_time!) &&
